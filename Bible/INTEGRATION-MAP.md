...
- **2026-07-20** `[FIX]` `[P0]` `[player]`: texture PBR + animazione runtime + zoom smoothing
  - **Impatto**: (1) `rig_final.py`: basecolor ora forzato `sRGB` (prima dipendeva da default Blender → muddy/dark in Godot). GLB `chr_player_rigged_anim.glb` re-esportato (3.6MB) con colorspace corretto (basecolor→sRGB, normal/rm→Non-Color). (2) `animation_tree_setup.gd`: rimosso `assert(animation_player_node != null)` (ora `push_error`+return null se null), aggiunto `tree_root = null` prima di riassegnare (workaround Godot 4.7 per registrare il tree a runtime), fix 2 warning `add_blend_point` con nomi espliciti. (3) `camera_controller.gd`: `zoom_speed` 1.0→0.5 (smoother). (4) `player.gd._ready()`: `Input.mouse_mode = Input.MOUSE_MODE_CAPTURED` (cursor nascosto/catturato). Verificato headless: ANIMTREE active=true, 9 anim caricate, nessun SCRIPT ERROR. Texture colorspace corretto (Blender pipeline conferma).
  - **Rischio**: basso. Vedi DECISIONS-LOG D013 + 03-TECHNICAL-BIBLE.md Edge Cases.
