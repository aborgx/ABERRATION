...
- **2026-07-20** `[FIX]` `[P0]` `[player]`: camera terza persona + playback animazioni idle + zoom
  - **Impatto**: (1) `camera_controller.gd`: rimossa `camera.position=Vector3(0,0,0)` (L40) e branch `else` lerp a ZERO (L102) che annullavano `global_position=target_pos+real_offset` (L83). `distance` 5→6, `height` 2→2.5. Aggiunto `min_distance=2/max_distance=12/zoom_speed=1` + zoom mouse wheel (`zoom_in`/`zoom_out` in project.godot, button 4/5) che adjusta `distance` con `clampf`. (2) `player.gd`: aggiunto `add_to_group("player")` in `_enter_tree()` (prima il `camera_controller` non trovava target → camera non si spostava). Rimosso `call_deferred` conflittuale per `set_active`/`playback.start()` (l'AnimationTree `_ready()` già gestisce `active=true`+`playback.start(&"Idle")`). Verificato headless: OFFSET.z=6.0 (dietro player), state machine vivo (`current_node=Fall` headless per no floor; Idle con floor), nessun SCRIPT ERROR. Zoom range 2-12m.
  - **Rischio**: basso. Vedi DECISIONS-LOG D012 + 03-TECHNICAL-BIBLE.md Edge Cases.
